...
const Equipment = () => {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      await apiClient.post("/api/equipment", newEquip);
  ...
  const markMissing = async (id) => {
    if (
      !window.confirm(
        "Are you sure you want to mark this item as missing? This will create an issue."
      )
    )
      return;
    try {
      ...
      <div className="bg-white p-4 rounded shadow mb-6">
        ...
        >
          ...
          <input
            ...
            onChange={(e) =>
              setNewEquip({ ...newEquip, serialNumber: e.target.value })
            }
          />
          <input
            type="text"
            ...
            onChange={(e) =>
              setNewEquip({ ...newEquip, notes: e.target.value })
            }
          />
          <button
            type="submit"
