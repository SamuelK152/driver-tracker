...
const DriverProfiles = () => {
  ...
  const fetchDrivers = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/driver-profiles', {
  ...
  return (
    <div className="p-4">
      ...
      <p className="mb-4 text-gray-600">Manage persistent driver preferences and training.</p>
          ...
          <table className="min-w-full bg-white border border-gray-300">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="py-2 px-4 border-b text-left">Preferred Equipment</th>
              </tr>
            </thead>
            <tbody>
              {drivers.map(driver => (
                <tr key={driver._id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4 border-b">
                    {driver.preferredEquipment?.map(e => e.serialNumber).join(', ') || '-'}
                  </td>
                </tr>
              ))}
              {drivers.length === 0 && (
                <tr>
                  <td colSpan="4" className="py-4 text-center text-gray-500">
                    No profiles found. Profiles are created when drivers are assigned.
                  </td>
                </tr>
              )}
            </tbody>
